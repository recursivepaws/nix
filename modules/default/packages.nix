...
{ den, lib, ... }:
let
  packageClass = { host, user, class, aspect-chain }:
    let
      ...
      overlay = final: prev:
        lib.foldl (acc: f:
          let
            available = prev // { pkgs = prev; };
            passedArgs =
              lib.filterAttrs (n: _: (builtins.functionArgs f) ? ${n})
              available;
          in acc // (f passedArgs)) { } pkgFns;
    in lib.optionalAttrs (pkgFns != [ ]) {
      nixos.nixpkgs.overlays = [ overlay ];
      homeManager.nixpkgs.overlays = [ overlay ];
    };

in { den.ctx.user.includes = [ packageClass ]; }
